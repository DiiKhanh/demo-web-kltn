...
def save_metrics_to_csv(filename, acc, prec, rec, f1):
    ...
    with open(filename, mode='a', newline='') as csv_file:
        ...
        # Ghi dòng dữ liệu
        writer.writerow([f"{acc:.4f}", f"{prec:.4f}", f"{rec:.4f}", f"{f1:.4f}"])

def train_test_split_stratify(data_folder, patient_ids, test_size = 0.2):
    num_patients = len(patient_ids)
    y = list()
    for patient_id in patient_ids:
        meta_data = load_text_file(os.path.join(data_folder, patient_id, f"{patient_id}.txt"))
        outcome = get_outcome(meta_data)
        y.append(outcome)
        
    if num_patients==0:
        raise FileNotFoundError('No data was provided.')

    training_ids, testing_ids = train_test_split(patient_ids, test_size=test_size, random_state=42, stratify=y)
...
def plot_metrics_from_csv(csv_file):
    ...
    # Vẽ AUC
    plt.subplot(1, 2, 2)
    plt.plot(epochs, df["train_auc"], label="Train AUC")
    plt.plot(epochs, df["val_auc"], label="Validation AUC")
    plt.xlabel("Epoch")
    plt.ylabel("AUC")
...
def log_metrics_to_csv(
    ...
    val_loss, val_auc, val_acc, val_f1, val_precision, val_recall,
    lr
):
    ...
    with open(csv_file, mode="a", newline="") as f:
        ...
        writer.writerow([
            epoch + 1,
            round(train_loss, 4), round(train_auc, 4),
            round(val_loss, 4), round(val_auc, 4), round(val_acc, 4),
            round(val_f1, 4), round(val_precision, 4), round(val_recall, 4),
            round(lr, 6)
        ])
...
def print_epoch_summary(
    epoch, 
    avg_train_loss, avg_train_auc, 
    avg_val_loss, avg_val_auc, 
    avg_val_acc, avg_val_f1, 
    avg_val_precision, avg_val_recall, 
    best_epoch, min_val_loss, best_auc, 
    current_lr
):
    print("="*80)
    print(f"Epoch {epoch + 1}")
    print("-" * 80)
    print(f"{'Train Loss':<15}: {avg_train_loss:.4f}")
    print(f"{'Train AUC':<15}: {avg_train_auc:.4f}")
    print(f"{'Val Loss':<15}: {avg_val_loss:.4f}")
    print(f"{'Val AUC':<15}: {avg_val_auc:.4f}")
    print(f"{'Val Accuracy':<15}: {avg_val_acc:.4f}")
    print(f"{'Val F1 Score':<15}: {avg_val_f1:.4f}")
    print(f"{'Val Precision':<15}: {avg_val_precision:.4f}")
    print(f"{'Val Recall':<15}: {avg_val_recall:.4f}")
    print("-" * 80)
    print(f"Best so far -> Epoch {best_epoch + 1} | Val Loss: {min_val_loss:.4f} | Val AUC: {best_auc:.4f}")
    print(f"Learning Rate       : {current_lr}")
    print("="*80)


def save_predictions_to_csv( prediction_results,folder, filename="challenge_predictions.csv"):
    csv_file = folder + '/'+filename
    # Xây header tự động từ keys
    fieldnames = [
        'PatientID', 'PredictedOutcome', 'OutcomeProbability','ActualOutcome'
    ]
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        for result in prediction_results:
            row = {
                ...
                'PredictedOutcome': 'Good' if sanitize_scalar_value(result['outcome_binary']) == 0 else 'Poor',
                'OutcomeProbability': sanitize_scalar_value(result['outcome_prob']), 
                'ActualOutcome': result['actual_outcome'],
            }
            writer.writerow(row)
            

def load_predictions_from_csv(folder, filename="challenge_predictions.csv"):
    results = []
    csv_file = folder + '/'+filename
    try:
        with open(csv_file, 'r') as f:
            ...
            for row in reader:
                result = {
                    ...
                    'outcome_binary': int(row['PredictedOutcome'] == 'Poor'),
                    'outcome_prob': float(row['OutcomeProbability']),
                    'actual_outcome': int(row['ActualOutcome'] == 'Poor'), 
                }
...
def save_signal_data_of_one_record(filepath, signal_data):
...
def read_signal_data_from_file(filepath):
